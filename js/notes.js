(() => {
  ...
  function render() {
    ...
    root.innerHTML = list.map(note => {
      ...
      const imageHtml = images.length ? `<div class="note-images count-${images.length}">${images.map((src, index) => `<img src="${escapeHtml(src)}" alt="${escapeHtml(note.title || "随笔图片")} ${index + 1}" loading="lazy">`).join("")}</div>` : "";
    ...
    root.querySelectorAll("[data-expand]").forEach(button => button.onclick = () => {
  ...
  async function load() {
    try {
      const response = await fetch(`./data/notes.json?v=${Date.now()}`, { cache: "no-store" });
      if (!response.ok) throw new Error("无法读取内容");
      const store = await response.json();
      notes = (store.notes || []).length ? store.notes.sort((a, b) => new Date(b.date) - new Date(a.date)) : defaults;
    } catch {
      notes = defaults;
    }
    render();
